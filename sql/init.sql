...
--
-- Lifecycle (two-sided handshake before any game starts):
...
--   complete   -> voided (admin reversed the result)
--
-- ONE open (pending/active) duel per league at a time, enforced at the DB
-- level by the generated `open_chat_id` column + its UNIQUE index (see below).
--
-- NOTE: this table is ALSO defined in sql/duels.sql + app/migrate_duels.py,
-- the migration path for leagues whose DB was created before duels existed.
...
CREATE TABLE IF NOT EXISTS duels (
  ...
  -- a concurrent second /duel fails the INSERT rather than racing through.
  open_chat_id          BIGINT
                        GENERATED ALWAYS AS (
